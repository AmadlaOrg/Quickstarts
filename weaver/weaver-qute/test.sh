...
if command -v "$PLUGIN" &>/dev/null; then
    ...
    PLUGIN_DIR="$(dirname "$SCRIPT_DIR")/../../weaver-qute"
    BIN="$PLUGIN_DIR/target/weaver-qute-1.0.0-runner"
    if [[ ! -x "$BIN" ]]; then
        echo "Building $PLUGIN native binary from source..."
        export JAVA_HOME="${GRAALVM_HOME:-${JAVA_HOME:-/home/jn/.local/graalvm}}"
        export GRAALVM_HOME="$JAVA_HOME"
        export PATH="$JAVA_HOME/bin:$PATH"
        (cd "$PLUGIN_DIR" && mvn package -Dnative -DskipTests -q) || { echo "ERROR: failed to build $PLUGIN"; exit 2; }
    fi
fi
...
if echo "$info_output" | grep -q '"engine" : "qute"'; then
...
run_test "nginx.conf (JSON input)" \
    "templates/nginx.conf.qute" "data/nginx.json" "expected/nginx.conf"

# Test 3: upstream.conf from YAML
run_test "upstream.conf (YAML input)" \
    "templates/upstream.conf.qute" "data/upstreams.yaml" "expected/upstream.conf"

# Test 4: systemd.service from YAML
run_test "systemd.service (YAML input)" \
    "templates/systemd.service.qute" "data/systemd.yaml" "expected/systemd.service"

# Test 5: stdin pipe
stdin_output=$(echo '{"server_name": "test.local", "listen_port": 443, "root_path": "/var/www/test", "proxy_port": 8080}' \
    | "$BIN" render -t "$SCRIPT_DIR/templates/nginx.conf.qute" -f -)
...
tmpfile=$(mktemp)
"$BIN" render -t "$SCRIPT_DIR/templates/nginx.conf.qute" -f "$SCRIPT_DIR/data/nginx.json" -o "$tmpfile"
