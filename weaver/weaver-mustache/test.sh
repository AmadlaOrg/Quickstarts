...
if command -v "$PLUGIN" &>/dev/null; then
    ...
    PLUGIN_DIR="$(dirname "$SCRIPT_DIR")/../../weaver-mustache"
    BIN="$(mktemp /tmp/weaver-mustache.XXXXXX)"
...
if echo "$info_output" | grep -q '"engine": "mustache"'; then
...
run_test "nginx.conf (JSON input)" \
    "templates/nginx.conf.mustache" "data/nginx.json" "expected/nginx.conf"

# Test 3: upstream.conf from YAML
run_test "upstream.conf (YAML input)" \
    "templates/upstream.conf.mustache" "data/upstreams.yaml" "expected/upstream.conf"

# Test 4: systemd.service from YAML
run_test "systemd.service (YAML input)" \
    "templates/systemd.service.mustache" "data/systemd.yaml" "expected/systemd.service"

# Test 5: stdin pipe
stdin_output=$(echo '{"server_name": "test.local", "listen_port": 443, "root_path": "/var/www/test", "proxy_port": 8080}' \
    | "$BIN" render -t "$SCRIPT_DIR/templates/nginx.conf.mustache" -f -)
...
tmpfile=$(mktemp)
"$BIN" render -t "$SCRIPT_DIR/templates/nginx.conf.mustache" -f "$SCRIPT_DIR/data/nginx.json" -o "$tmpfile"
