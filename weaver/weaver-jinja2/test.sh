...
if command -v "$PLUGIN" &>/dev/null; then
    ...
    # Fallback to venv in plugin source directory
    PLUGIN_DIR="$(dirname "$SCRIPT_DIR")/../../weaver-jinja2"
    BIN="$PLUGIN_DIR/.venv/bin/$PLUGIN"
    if [[ ! -x "$BIN" ]]; then
        echo "ERROR: $PLUGIN not found in PATH or $BIN"
        exit 2
    fi
fi
...
if echo "$info_output" | grep -q '"engine": "jinja2"'; then
...
run_test "nginx.conf (JSON input)" \
    "templates/nginx.conf.j2" "data/nginx.json" "expected/nginx.conf"

# Test 3: upstream.conf from YAML
run_test "upstream.conf (YAML input)" \
    "templates/upstream.conf.j2" "data/upstreams.yaml" "expected/upstream.conf"

# Test 4: systemd.service from YAML
run_test "systemd.service (YAML input)" \
    "templates/systemd.service.j2" "data/systemd.yaml" "expected/systemd.service"

# Test 5: stdin pipe
stdin_output=$(echo '{"server_name": "test.local", "listen_port": 443, "root_path": "/var/www/test", "proxy_port": 8080}' \
    | "$BIN" render -t "$SCRIPT_DIR/templates/nginx.conf.j2" -f -)
...
tmpfile=$(mktemp)
"$BIN" render -t "$SCRIPT_DIR/templates/nginx.conf.j2" -f "$SCRIPT_DIR/data/nginx.json" -o "$tmpfile"
